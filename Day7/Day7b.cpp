...
private: 
    ...
    int get_max()
    {
        ...
        for (map<char, int>::iterator it = hand_card_amounts.begin(); it != hand_card_amounts.end(); it++)
        {
            if (it->first == 'J')
            {
                // can't add jokers to jokers
                current_max = max(current_max, it->second); 
                continue;
            }
            current_max = max(current_max, it->second + joker_amount);
        }
    ...
    int get_min()
    {
        ...
        for (map<char, int>::iterator it = hand_card_amounts.begin(); it != hand_card_amounts.end(); it++)
        {
            if (it->first == 'J')
            {
                // we don't want the minimum to decrease
                // if we only have 1 joker and 2 pairs
                // this would make a full house with a joker impossible.
                continue;
            }
    ...
    int get_pair_amount()
    {
        ...
        for (map<char, int>::iterator it = hand_card_amounts.begin(); it != hand_card_amounts.end(); it++)
        {
            ...
            {
                amount++;
                // no need to continue, as the jokers would
                // already have been used to make a three-pair
            }

            if (joker_amount > 0 && it->first != 'J')
            {
                amount++;

                // we can't reuse jokers
                joker_amount -= 1;
            }
        }
    ...
    void calculate_value()
    {
        ...
        {
            if (hand[i] == 'J') { joker_amount++; }

            hand_card_amounts[hand[i]] += 1;
        }

        // first ordering
        if (get_max() == 5) { value = 6; }
